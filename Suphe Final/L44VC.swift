...
class L44VC: UIViewController {

    let userArray = ["level","Level","LEVEL","seviye","Seviye","SEVIYE","SEVİYE"]
    let passArray =  ["43","kırküç","Kırküç","Kirkuc","KIRKÜÇ","KİRKÜÇ","KIRKUC"]
    ...
    @IBAction func enterPressed(_ sender: UIButton) {
        if userArray.contains(userTextField.text!){
            if passArray.contains(passTextField.text!){
                ...
                performSegue(withIdentifier: "to44_1", sender: self)
                playSuccess()
