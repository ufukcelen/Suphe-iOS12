...
//  Şüphe
//
//  Created by Ufuk Celen on 25.07.2018.
//  Copyright © 2018 Ufuk Celen. All rights reserved.
//

import UIKit

class L39VC: UIViewController {

    let userArray = ["sinestezi","Sinestezi","SİNESTEZİ","SINESTEZI"]
    ...
    @IBAction func enterPressed(_ sender: UIButton) {
        if userArray.contains(passTextField.text!) {
            performSegue(withIdentifier: "to40", sender: nil)
            playSuccess()
        }else{
            passTextField.text = ""
            playError()
