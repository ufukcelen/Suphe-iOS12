...
//  Şüphe
//
//  Created by Ufuk Celen on 13.07.2018.
//  Copyright © 2018 Ufuk Celen. All rights reserved.
//

import UIKit

class L26VC: UIViewController {

    let userArray = ["ekinoks","Ekinoks","EKİNOKS","EKINOKS"]
    ...
    @IBAction func enterPressed(_ sender: UIButton) {
        if userArray.contains(passTextField.text!) {
            performSegue(withIdentifier: "to27", sender: nil)
            playSuccess()
        }else{
            passTextField.text = ""
            playError()
