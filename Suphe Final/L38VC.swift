...
//  Şüphe
//
//  Created by Ufuk Celen on 23.07.2018.
//  Copyright © 2018 Ufuk Celen. All rights reserved.
//

import UIKit

class L38VC: UIViewController {

    let userArray =  ["deja","Deja","DEJA"]

    let passArray = ["vu","Vu","VU"]
    ...
    @IBAction func enterPressed(_ sender: UIButton) {
        if userArray.contains(userTextField.text!){
            if passArray.contains(passTextField.text!){
                ...
                performSegue(withIdentifier: "to39", sender: self)
                playSuccess()
