...
class L45VC: UIViewController {

    let userArray =  ["deadly","Deadly","DEADLY"]
    let passArray = ["nightshade","Nightshade","NİGHTSHADE","NIGHTSHADE"]
    ...
    @IBAction func enterPressed(_ sender: UIButton) {
        if userArray.contains(userTextField.text!){
            if passArray.contains(passTextField.text!){
                ...
                performSegue(withIdentifier: "to46", sender: self)
                playSuccess()
